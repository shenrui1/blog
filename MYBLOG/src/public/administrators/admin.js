$(function () {
    ...
    getAllBlog.addEventListener('mouseover', function () {
        $(getAllBlog).css('cursor', 'pointer');
    });
    var _loop_1 = function (j) {
        getUser[j].addEventListener('mouseover', function () {
    ...
    var _loop_2 = function (k) {
        getadministrator[k].addEventListener('mouseover', function () {
            $(getadministrator[k]).css('cursor', 'pointer');
        });
    };
    for (var k = 0; k < getadministrator.length; k++) {
        _loop_2(k);
    }
    getAllUser.addEventListener('click', function () {
        for (var i = 0; i < getMainContent.length; i++) {
            $(getMainContent[i]).hide();
        }
        $(getMainContent[0]).show();
    });
    getAllBlog.addEventListener('click', function () {
        for (var i = 0; i < getMainContent.length; i++) {
            $(getMainContent[i]).hide();
        }
        $(getMainContent[1]).show();
    });
    getUser[0].addEventListener('click', function () {
        for (var i = 0; i < getMainContent.length; i++) {
            $(getMainContent[i]).hide();
        }
        $(getMainContent[2]).show();
    });
    getUser[1].addEventListener('click', function () {
        for (var i = 0; i < getMainContent.length; i++) {
            $(getMainContent[i]).hide();
        }
        $(getMainContent[3]).show();
    });
    getadministrator[0].addEventListener('click', function () {
        for (var i = 0; i < getMainContent.length; i++) {
            $(getMainContent[i]).hide();
        }
        $(getMainContent[4]).show();
    });
    getadministrator[1].addEventListener('click', function () {
        for (var i = 0; i < getMainContent.length; i++) {
            $(getMainContent[i]).hide();
        }
        $(getMainContent[5]).show();
    });
    getadministrator[2].addEventListener('click', function () {
        for (var i = 0; i < getMainContent.length; i++) {
            $(getMainContent[i]).hide();
        }
    ...
    // 删除用户事件
    $('#delUser').on('submit', function () {
        var getForm = formToDic($('#delUser'));
        if ($.trim(getForm.userName).length == 0) {
            alert('请输入用户名');
            return false;
        }
    ...
    // 查询文章事件
    $('#searchBlog').on('submit', function () {
        var getForm = formToDic($('#searchBlog'));
        var getName = $('input:checkbox[name="blogClass"]:checked');
        var getbbb = [];
        for (var i = 0; i < getName.length; i++) {
            getbbb.push(getName[i].value);
        }
    ...
    // 删除文章事件
    $('#delBlog').on('submit', function () {
        var getForm = formToDic($('#delBlog'));
        if ($.trim(getForm.blogName).length == 0) {
            alert('请输入文章名');
            return false;
        }
